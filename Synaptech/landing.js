...
function submitForm(e, formId) {
  ...
  fetch('https://n8n-yvdu.srv1552695.hstgr.cloud/webhook/synaptech', {
    ...
    body: JSON.stringify({
      ...
      whatsapp: wpp,
      solucao:  sol,
      origem:   formId === 'top' ? 'formulario_topo' : 'formulario_final',
  ...
  .then(res => {
    ...
    // Meta Pixel
    if (typeof fbq !== 'undefined') fbq('track', 'Lead', { content_name: sol });
    // Mostra sucesso
  ...
  .catch(() => {
    ...
    // e tenta novamente em segundo plano
    form.style.display = 'none';
    success.style.display = 'flex';
    ...
    setTimeout(() => {
      fetch('https://n8n-yvdu.srv1552695.hstgr.cloud/webhook/synaptech', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ nome, whatsapp: wpp, solucao: sol, origem: formId === 'top' ? 'formulario_topo' : 'formulario_final', pagina: window.location.href, data: new Date().toISOString(), retry: true }),
      }).catch(() => {});
    }, 3000);
  })
;
